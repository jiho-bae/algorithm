...
function solution(n, time) {
  ...
  for (let i = 0; i < n; i++) {
    deliveryTime.push(time.shift());
  }
  let cnt = 0;

  while (time.length !== 0 || deliveryTime.length !== 0) {
    deliveryTime = deliveryTime.map((time) => time - 1);
    while (deliveryTime.includes(0)) {
      let idx = deliveryTime.indexOf(0);
      if (time.length === 0) {
        deliveryTime.splice(idx, 1);
      } else deliveryTime[idx] = time.shift();
    }
    cnt++;
  }
  return cnt;
